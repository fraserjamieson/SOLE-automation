...
describe('This is a test', () => {
  ...
  it('Can hide/show address', () => {
    ...
    cy.visit('/');
    cy.adminLogin('stasoletesting+garden@gmail.com', 'letitsnow1');
    ...
    cy.visit('https://test.sole.scot/dunbar/garden-yard-s-supplies/detail');
  ...
  it('Can search for specific community post, logged in', () => {
    ...
    cy.get(`input[type='email']`).type('dunbar@demo.com');
    cy.get('#psd').type('test1234');
  ...
  it.only('Can search for specific community post and like it, logged in', () => {
    cy.visit(Cypress.env('cstUrl'));
    cy.get('.header_menu').click();
    cy.get(`input[type='email']`).type('dunbar@demo.com');
    cy.get('#psd').type('test1234');
